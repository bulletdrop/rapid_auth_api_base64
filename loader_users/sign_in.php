...
switch (false)
{
    ...
    case check_creds($username, $password, $gid):
        echo base64_encode(json_encode(array("status" => "error", "message" => "Invalid username or password")));
        break;
    case hwid_exist($username, $password, $gid):
        insert_hwid($username, $password, $gid, $hwid);
        update_last_ip_address($username, $password, $gid);
        echo base64_encode(json_encode(array("status" => "success", "message" => "Successfully signed in")));
        break;
    case verify_hwid($username, $password, $gid, $hwid):
        update_hwid_attempt($username, $password, $gid, $hwid);
        update_last_ip_address($username, $password, $gid);
        echo base64_encode(json_encode(array("status" => "error", "message" => "Invalid hardware ID")));
        break;        
    default:
        ...
        {
            echo base64_encode(json_encode(array("status" => "error", "message" => "Unknow error")));  
        }
        break;  
}
